...
void solve()
{
    ...
    ll low = maxInA, high = maxInA + 2 * sum;

    // Binary search for the minimum possible value
    while (low < high)
    {
        ll mid = low + ((high - low) >> 1);

        __int128 Observed = 0, Comp = 0;

        for (int i = 0; i < N; i++)
        {
            __int128 d = (__int128)mid - A[i];
            __int128 u = (Brr[i] < d) ? Brr[i] : d;
