...
void solve()
{
    ...
    inlarr(vitamins,n);
    inlarr(costs,n);
    unordered_map<ll,ll> minCostVitamin;
    for(int i=0;i<n;i++){
        if(minCostVitamin.find(vitamins[i]) == minCostVitamin.end() || costs[i] < minCostVitamin[vitamins[i]]) {
            minCostVitamin[vitamins[i]] = costs[i];
        }
    }
    ll ans=0;
    for(auto& it:minCostVitamin){
        if(effectivness-it.second>0){
            ans+=effectivness-it.second;
        }
    }
    out(ans);
}
